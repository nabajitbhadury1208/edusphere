...
public class CourseController {
    ...
    public ResponseEntity<String> setActivate(@PathVariable UUID id, @Valid @RequestBody  Status status) {
        try {
            courseService.setActivateDeactivate(id, status);
            return ResponseEntity.ok("Successfully activated/deactivated User with Id: " + id);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
